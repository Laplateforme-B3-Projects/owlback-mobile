...
export function CustomGlassView({
  ...
  isExpenseView = false
}: { className?: string; isDocumentGlassStyle?: boolean; isExpenseView?: boolean }) {
  ...
  return (<>{
    ...
    (
      ...
      <BlurView
        ...
        className={cn(!isDocumentGlassStyle && !isExpenseView ? "rounded-full": "rounded-xl", " border border-zinc-200/50", className)}
      />
    )
    }</>
  );
